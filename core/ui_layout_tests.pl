...
lay(El, W-H, Layout) :-
    build(El, Node),
    root(W, H, Node, Root),
    layout_tree(defer_measurement, Root, Layout).
...
test('an empty tree has no layout') :-
    ...
    layout_tree(defer_measurement, Root, L),
    L == none.
...
test('a bounded scroll container overflows while its box stays at the viewport') :-
    lay(div([overflow([main])], [div([main_size(80), cross_size(10)], []),
                                 div([main_size(80), cross_size(10)], [])]),
        100-100, L),
    u(100, H), get_dict(height, L, H),
    get_dict(scroll, L, [main]),
    u(60, O), get_dict(overflow, L, O).

test('growth is inert on the scroll axis') :-
    lay(div([direction(row), overflow([main])],
            [div([flex(1), main_size(30), cross_size(10)], []),
             div([flex(1), main_size(30), cross_size(10)], [])]),
        40-50, L),
    u(30, W),
    get_dict(children, L, [child(0, _, A), child(_, _, B)]),
    get_dict(width, A, W),
    get_dict(width, B, W),
    u(20, O), get_dict(overflow, L, O),
    get_dict(scroll, L, [main]).

test('flex under an unbounded main does not throw in a scroll container') :-
    lay(div([], [div([overflow([main])],
                     [div([flex(1), main_size(10), cross_size(10)], [])])]),
        100-50, L),
    get_dict(children, L, [child(_, _, Mid)]),
    get_dict(scroll, Mid, [main]),
    \+ get_dict(overflow, Mid, _),
    u(10, H), get_dict(height, Mid, H).

test('a fitting scroll container is marked but carries no overflow key') :-
    lay(div([overflow([main])], [div([main_size(10), cross_size(10)], [])]), 100-100, L),
    get_dict(scroll, L, [main]),
    \+ get_dict(overflow, L, _).
...
test('an identical relayout reuses the whole tree') :-
    ...
    root(100, 100, Node, Root),
    layout_tree(defer_measurement, Root, L0),
    relayout_tree(defer_measurement, [], Root, L0, L1),
    same_term(L0, L1).
...
test('a non-layout attribute change reuses the whole tree') :-
    ...
    layout_tree(defer_measurement, Root0, L0),
    root(100, 100, Node1, Root1),
    relayout_tree(defer_measurement, Changes, Root1, L0, L1),
    same_term(L0, L1).
...
test('a viewport change reuses unaffected children') :-
    ...
    relayout_tree(defer_measurement, [], Root1, L0, L1),
    \+ same_term(L0, L1),
...
test('incremental relayout equals a fresh layout') :-
    ...
    relayout_tree(defer_measurement, Changes, Root1, L0, L1),
    layout_tree(defer_measurement, Root1, LFresh),
    L1 == LFresh.
