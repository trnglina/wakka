:- module(ui_layout, [ layout_tree/3, relayout_tree/5, defer_measurement/2, px_units/2 ]).
...
%! layout_tree(:MeasureContent, +Root, -Layout) is det.

layout_tree(MeasureContent, Root, Layout) :-
    relayout_tree(MeasureContent, [], Root, none, Layout).

%! relayout_tree(:MeasureContent, +Changes, +Root, +PrevLayout, -Layout) is det.

relayout_tree(MeasureContent, Changes, Root, PrevLayout, Layout) :-
    ...
    ;  changes_dirty(Changes, Dirty),
       node_layout(MeasureContent, Dirty, [], Node, constraints(W, W, H, H), PrevLayout, Layout)
    ).

%! defer_measurement(+Request, -Metrics) is failure.
%
%  Default content measurer: measures nothing, so every inline becomes a
%  placeholder at its minimum constrained size carrying its pending
%  measurement request for a later pass. Hosts supply a real MeasureContent
%  to answer these requests.

defer_measurement(_, _) :- fail.
...
%! node_layout(:MeasureContent, +Dirty, +Path, +Node, +Constraints, +Prev, -Layout) is det.

node_layout(MeasureContent, Dirty, Path, Node, Constraints, Prev, Layout) :-
    (  is_dict(Prev),
    ...
    -> inline_options_(attrs{}, InlineOptions),
       inline_layout(MeasureContent, Dirty, Path, Node, InlineOptions, Constraints, none, Layout)
    ;  block_layout(MeasureContent, Dirty, Path, Node, Constraints, Prev, Layout)
    ).

%! block_layout(:MeasureContent, +Dirty, +Path, +Node, +Constraints, +Prev, -Layout) is det.
...
block_layout(MeasureContent, Dirty, Path, Node, Constraints, Prev, Layout) :-
    ...
    PadW is PL + PR,
    PadH is PT + PB,
    direction_(Dir, MinW, MinH, MinMain, MinCross),
    direction_(Dir, MaxW, MaxH, MaxMain, MaxCross),
    direction_(Dir, PadW, PadH, PadMain, PadCross),
    bounded_sub_(MaxCross, PadCross, ContentCrossMax),
    overflow_axes_(Attrs, OverflowAxes),
    overflow_axis_(OverflowAxes, main, ScrollMain),
    flow_items_(Children, 0, Items),
    Ctx = ctx(MeasureContent, Dirty, Path, Prev, Dir, CrossAlign, ContentCrossMax, InlineOptions),
    size_rigid_items_(Items, Ctx, ScrollMain, RigidSum, TotalFlex, RigidSized),
    (  TotalFlex > 0
    -> main_extent_(MaxMain, PadMain, Path, MainRoom),
       Remaining is max(0, MainRoom - RigidSum),
       size_flex_items_(RigidSized, Ctx, Remaining, TotalFlex, Sized)
    ;  Sized = RigidSized
    ),
    extents_sum_(Sized, SumExtents),
    (  TotalFlex > 0
    -> SelfMain is MainRoom + PadMain
    ;  SelfMain is min(max(SumExtents + PadMain, MinMain), MaxMain)
    ),
    max_cross_extent_(Sized, MaxChildCross),
    ...
    direction_(Dir, PL, PT, PadMainLead, PadCrossLead),
    CrossRoom is SelfCross - PadCross,
    PCtx = place(MainAlign, Free, NItems, PadMainLead, PadCrossLead, CrossRoom, CrossAlign, Dir),
    place_items_(Sized, 0, 0, PCtx, Placed),
    direction_(Dir, Width, Height, SelfMain, SelfCross),
    Layout1 = layout{width: Width, height: Height, children: Placed,
                     constraints: Constraints, path: Path},
    Overflow is SumExtents - ContentMain,
    (  Overflow > 0
    -> put_dict(overflow, Layout1, Overflow, Layout2)
    ;  Layout2 = Layout1
    ),
    (  OverflowAxes \== []
    -> put_dict(scroll, Layout2, OverflowAxes, Layout)
    ;  Layout = Layout2
    ).
...
%! size_rigid_items_(+Items, +Ctx, +ScrollMain, -RigidSum, -TotalFlex, -Sized) is det.

size_rigid_items_([], _, _, 0, 0, []).
size_rigid_items_([Item|Items], Ctx, ScrollMain, RigidSum, TotalFlex, [M|Ms]) :-
    effective_flex_(Item, ScrollMain, Flex),
    (  Flex > 0
    -> M = pending_flex(Item, Flex),
       size_rigid_items_(Items, Ctx, ScrollMain, RigidSum, TotalFlex0, Ms),
       TotalFlex is TotalFlex0 + Flex
    ;  size_item_(Item, Ctx, unbounded, M),
       sized_extent_(M, Extent),
       size_rigid_items_(Items, Ctx, ScrollMain, RigidSum0, TotalFlex, Ms),
       RigidSum is RigidSum0 + Extent
    ).

%! effective_flex_(+Item, +ScrollMain, -Flex) is det.

effective_flex_(_, true, 0) :- !.
effective_flex_(Item, false, Flex) :-
    item_flex_(Item, Flex).

%! size_flex_items_(+RigidSized, +Ctx, +Remaining, +TotalFlex, -Sized) is det.
%
...
size_flex_items_(RigidSized, Ctx, Remaining, TotalFlex, Sized) :-
    size_flex_items_(RigidSized, Ctx, Remaining, TotalFlex, 0, 0, Sized).

size_flex_items_([], _, _, _, _, _, []).
size_flex_items_([pending_flex(Item, Flex)|RMs], Ctx, Remaining, TotalFlex, PrevCumFlex, Alloc,
                 [M|FMs]) :- !,
    CumFlex is PrevCumFlex + Flex,
    Boundary is Remaining * CumFlex // TotalFlex,
    Share is Boundary - Alloc,
    item_fit_(Item, Fit),
    MainSpec =.. [Fit, Share],
    size_item_(Item, Ctx, MainSpec, M),
    size_flex_items_(RMs, Ctx, Remaining, TotalFlex, CumFlex, Boundary, FMs).
size_flex_items_([M|RMs], Ctx, Remaining, TotalFlex, CumFlex, Alloc, [M|FMs]) :-
    size_flex_items_(RMs, Ctx, Remaining, TotalFlex, CumFlex, Alloc, FMs).
...
%! size_item_(+Item, +Ctx, +MainSpec, -Sized) is det.
%
%  Sized = sized(Lead, Trail, CrossLead, CrossTrail, MainSize, CrossSize, Layout)
...
size_item_(block(Idx, Child), Ctx, MainSpec,
           sized(Lead, Trail, CrossLead, CrossTrail, MainSize, CrossSize, ChildLayout)) :-
    Ctx = ctx(MeasureContent, Dirty, Path, Prev, Dir, CrossAlign, ContentCrossMax, _),
    get_dict(attributes, Child, Attrs),
    box_sides_(Attrs, margin, MT, MR, MB, ML),
    direction_(Dir, ML, MT, Lead, CrossLead),
    direction_(Dir, MR, MB, Trail, CrossTrail),
    ...
    direction_(Dir, CMinW, CMinH, CMinMain, CMinCross),
    direction_(Dir, CMaxW, CMaxH, CMaxMain, CMaxCross),
    append(Path, [Idx], ChildPath),
    prev_item_layout_(Prev, Idx, PrevChild),
    node_layout(MeasureContent, Dirty, ChildPath, Child,
    ...
    direction_(Dir, ChildW, ChildH, MainSize, CrossSize).
size_item_(inline(Idx, Node), Ctx, MainSpec,
           sized(0, 0, 0, 0, MainSize, CrossSize, InlineLayout)) :-
    Ctx = ctx(MeasureContent, Dirty, Path, Prev, Dir, CrossAlign, ContentCrossMax, Options),
    ...
    append(Path, [Idx], ChildPath),
    prev_item_layout_(Prev, Idx, PrevChild),
    inline_layout(MeasureContent, Dirty, ChildPath, Node, Options,
    ...
    direction_(Dir, InlineW, InlineH, MainSize, CrossSize).
...
sized_extent_(sized(Lead, Trail, _, _, MainSize, _, _), Extent) :-
...
max_cross_extent_([], 0).
max_cross_extent_([sized(_, _, CrossLead, CrossTrail, _, CrossSize, _)|Ms], Max) :-
    max_cross_extent_(Ms, Max0),
    Max is max(Max0, CrossLead + CrossSize + CrossTrail).

% --- Item Placement --- %

%! place_items_(+Sized, +Idx, +ExtentsBefore, +PCtx, -Children) is det.

place_items_([], _, _, _, []).
place_items_([sized(Lead, Trail, CrossLead, CrossTrail, MainSize, CrossSize, ChildLayout)|Ms],
    ...
    direction_(Dir, X, Y, MainPos, CrossPos),
...
%! inline_layout(:MeasureContent, +Dirty, +Path, +Node, +Options, +Constraints, +Prev, -Layout) is det.
%
...
%  forces the inline's box regardless of its content. When MeasureContent fails,
%  the inline becomes a placeholder at its minimum constrained size
%  carrying the request under the pending key.

inline_layout(MeasureContent, Dirty, Path, Node, Options, Constraints, Prev, Layout) :-
    (  is_dict(Prev),
    ...
    ;  Constraints = constraints(MinW, MaxW, MinH, MaxH),
       ...
       Request = measure_inline(Runs, Options, MaxW),
       (  call(MeasureContent, Request, metrics(W0, H0))
       -> Wc is ceiling(W0),
          Hc is ceiling(H0),
...
%! direction_(+Dir, ?W, ?H, ?Main, ?Cross) is det.

direction_(row, W, H, W, H).
direction_(column, W, H, H, W).

%! overflow_axes_(+Attrs, -OverflowAxes) is det.

overflow_axes_(Attrs, OverflowAxes) :-
    (  get_dict(overflow, Attrs, [OverflowAxes]),
       is_list(OverflowAxes)
    -> true
    ;  OverflowAxes = []  ).

%! overflow_axis_(+OverflowAxes, +Axis, -Overflow) is det.

overflow_axis_(OverflowAxes, Axis, Overflow) :-
    (  memberchk(Axis, OverflowAxes)
    -> Overflow = true
    ;  Overflow = false  ).
